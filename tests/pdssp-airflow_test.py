...
def test_name():
    name = pdssp-airflow.__name_soft__
    assert name == "pdssp-airflow"
...
def test_new_level():
    pdssp-airflow.custom_logging.UtilsLogs.add_logging_level("TRACE", 20)
...
def test_message():
    record = pdssp-airflow.custom_logging.LogRecord(
        "__main__",
        logging.INFO,
        "pathname",
        2,
...
def test_color_formatter():
    formatter = pdssp-airflow.custom_logging.CustomColorFormatter()
    logger = logging.getLogger("pdssp-airflow.custom_logging")
    ...
    shell_formatter = pdssp-airflow.custom_logging.ShellColorFormatter()
    record = pdssp-airflow.custom_logging.LogRecord(
        "__main__",
        logging.INFO,
        "pathname",
